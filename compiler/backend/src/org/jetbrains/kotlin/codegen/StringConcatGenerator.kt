...
class StringConcatGenerator(val mode: JvmStringConcat, val mv: InstructionAdapter) {
    ...
    data class Item(val type: Type, val itemType: ItemType, val value: String) {
        companion object {
            fun constant(value: String) = Item(JAVA_STRING_TYPE, ItemType.CONSTANT, value)
            fun bootstrap(value: String) = Item(JAVA_STRING_TYPE, ItemType.BOOTSTRAP, value)
            ...
            when (stackValue) {
                is StackValue.Constant -> {
                    ...
                    if (value is String && (value.contains("\u0001") || value.contains("\u0002"))) {
                        items.add(Item.bootstrap(value)) //strings with special symbols generated via bootstrap
                    } else if (value is Char && (value == 1.toChar() || value == 2.toChar())) {
                        items.add(Item.bootstrap(value.toString())) // //strings with special symbols generated via bootstrap
                    } else {
                        items.add(Item.constant(value.toString()))
                    }
                    return
                }
                TRUE -> {
                    items.add(Item.constant(true.toString()))
                    return
                }
                FALSE -> {
                    items.add(Item.constant(false.toString()))
                    return
                }
            }
        }
    ...
    fun genToString() {
        if (!mode.isDynamic) {
            ...
            if (mode == JvmStringConcat.INDY_WITH_CONSTANTS) {
                ...
                val specialSymbolsInTemplate = items.filter { it.itemType == ItemType.BOOTSTRAP }.map { it.value }
